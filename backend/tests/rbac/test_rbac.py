...
def test_admin_can_create_society(client, db):
    admin = make_user(db, "admin@rbac.com", role="Society Admin")
...
def test_committee_can_update_society(client, db):
    admin    = make_user(db, "admin2@rbac.com", role="Society Admin")
    society  = make_society(db, "Update Society")
    committee = make_user(db, "committee@rbac.com", role="Committee Chairman")
...
def test_security_cannot_update_society(client, db):
    ...
    security = make_user(db, "security@rbac.com", role="Security Staff")
    r = client.patch(f"/api/v1/societies/{society.id}", json={
        "city": "Delhi"
...
def test_admin_can_list_users(client, db):
    admin = make_user(db, "admin3@rbac.com", role="Society Admin")
...
def test_admin_dashboard_requires_admin(client, db):
    ...
    admin = make_user(db, "admin4@rbac.com", role="Society Admin")
    r2    = client.get("/api/v1/users/admin/dashboard", headers=admin["headers"])
    assert r2.status_code == 200


def test_multi_role_access(client, db):
    """User with Committee Chairman role should pass committee_or_admin guard."""
    from app.models.user import User, UserRole
    from app.models.role import Role
    from app.core.security import create_access_token

    r_admin = db.query(Role).filter(Role.name == "Society Admin").first()
    r_comm  = db.query(Role).filter(Role.name == "Committee Chairman").first()
    if not r_admin:
        r_admin = Role(name="Society Admin"); db.add(r_admin); db.flush()
    if not r_comm:
        r_comm = Role(name="Committee Chairman"); db.add(r_comm); db.flush()
    ...
    token   = create_access_token(str(user.id), {"roles": ["Society Admin", "Committee Chairman"]})
