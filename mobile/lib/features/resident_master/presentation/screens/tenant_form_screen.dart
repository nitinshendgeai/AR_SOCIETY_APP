  ...
  void _submit() {
    ...
    if (!_isEdit && (_agreementStart == null) != (_agreementEnd == null)) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content:
              Text('Provide both agreement start and end dates, or neither')));
      return;
    }
    ...
    if (_isEdit) {
      ...
      final data = <String, dynamic>{
        ...
        if (_agreementStart != null)
          'agreement_start_date': _dateStr(_agreementStart!),
        if (_agreementEnd != null)
          'agreement_end_date': _dateStr(_agreementEnd!),
        if (_rentCtrl.text.trim().isNotEmpty)
          'monthly_rent': _rentCtrl.text.trim(),
        if (_depositCtrl.text.trim().isNotEmpty)
          'security_deposit': _depositCtrl.text.trim(),
                    ...
                    child: RmFormField(
                      ...
                      child: InkWell(
                        onTap: () async {
                          ...
                          if (picked != null)
                            setState(() => _agreementStart = picked);
                        },
              ...
              RmFormField(
                ...
                child: DropdownButtonFormField<PoliceVerificationStatus>(
                  ...
                  items: PoliceVerificationStatus.values
                      ...
                      .toList(),
                  onChanged: (v) => setState(() =>
                      _policeStatus = v ?? PoliceVerificationStatus.pending),
                ),
              ),
              RmFormField(
                label: 'ID Proof Type',
